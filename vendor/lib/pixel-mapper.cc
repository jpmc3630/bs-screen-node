...
class FlipperPixelMapper : public PixelMapper {
public:
  FlipperPixelMapper() : angle_(0) {}

  virtual const char *GetName() const { return "Flipper"; }

  virtual bool GetSizeMapping(int matrix_width, int matrix_height,
                              int *visible_width, int *visible_height) const {
    *visible_height = 32;
    *visible_width = 112;   // because our square we cover is one panel wider
    return true;
  }
  ...
  void MapVisibleToMatrix(int matrix_width, int matrix_height,
    ...
    *matrix_x = visible_x;
    ...
    if (visible_y > 15) {
      *matrix_y += 32; // getting rid of the repeat stuff, cut it off
    } else if (visible_x > 63)  { 
      *matrix_x = 112 - (visible_x - 63); // rotate it
      *matrix_y = 16 - (visible_y + 1); // rotate it
      *matrix_y += 16;   // shift down
      *matrix_x -= 48;   // shift left
    }
  }
...
static MapperByName *CreateMapperMap() {
  ...
  RegisterPixelMapperInternal(result, new FlipperPixelMapper());
